...
describe('OrderService', () => {
  ...
  it('should retrieve orders from Api via GET', () => {
      const dummyData: any = [
        {
        assignTo: {azureUserId: '9876543', displayName: 'mark'},
        campus: 'ELL',
        category: 'Drank',
 ...
 */
  it('should retrieve orders By id from Api via GET', () => {
    ...
    service.getOrder(Id).subscribe((res: Order) => {
      console.log(res);
      expect(res).toEqual(dummyData2[1]);
  });

    const request = httpMock.expectOne(`${service.URL}/findOrder/${Id}`);
 ...
 */
  ...
  service.getMyOrders(requesterId).subscribe((res: Order[]) => {
    console.log(res);
    expect(res).toEqual(dummyData2[1]);
});

  const request = httpMock.expectOne(`${service.URL}/findByRequesterId/${requesterId}`);
 ...
 */
  ...
  service.getOrdersAssignToMe().subscribe((res: Order[]) => {
    console.log(res);
    expect(res).toEqual(dummyData2[1]);
});
  const  userid = service.userid;
  const request = httpMock.expectOne(`${service.URL}/orders/findByAssignToId/${userid}`);
